...
class OMP():
    ...
    def Cal_OneX_alpha_OMP(self, inputX, L, boolRound = False):
        ...
        # D 轉換區
        unusedD = np.matrix(self.D.copy())
        usedD  = np.matrix(np.zeros_like(unusedD)) 
        for i in range(L):
            #算相似度
            correlated = np.dot(residual.T, unusedD)
            if boolRound:
                correlated = np.round(correlated, 4)
            d_i = np.abs(correlated).argmax()
            print( d_i+1, "from", correlated[0])
            print("====>>>>", correlated.shape, alpha.shape, usedD.shape, unusedD.shape)
            # 使用過不再使用
            usedD[:, d_i] = unusedD[:, d_i].copy()
            unusedD[:, d_i].fill(0)
            print("usedD", *usedD, sep="\n")
            # 重新計算比例
#            alpha = np.linalg.pinv(usedD.T * usedD) * usedD.T * inputX
            alpha = np.linalg.lstsq(usedD, inputX) [0]
            print("alpha",alpha)
            # 殘差計算
            residual = inputX - self.D * alpha  #[:, d_i]
            if boolRound:
                residual = np.round(residual, 4)
            print("new-residual:", residual.T)
