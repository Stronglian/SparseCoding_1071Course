...
#        print(alpha)
#        print(data.shape, alpha.shape , self.beta, self.D.T.shape , np.dot(self.D , alpha).shape, (np.dot(self.D , alpha) - data).shape)
...
#        print("alpha", alpha, "absTmp", absTmp, "zeroMatrix", zeroMatrix, sep = '\n')
        maxTmp = np.max( [absTmp, zeroMatrix], axis = 0)
#        maxTmp = np.maximum( absTmp, 0)
        ...
        dif = oldCode - alpha
        stop = np.sqrt(dif.T * dif)
#        stop = self.normDist(dif)
...
#        for i, data in enumerate(self.Y):
        for i in range(self.Y.shape[1]):
            ...
            tmpAlpha = self.FLOW_oneAlpha(self.Y[:,i], convergeThreshold, max_iter)
...
#        tmp = ['#']
#        tmp.extend(["x"+str(i+1)+"'s SC" for i in range(Y.shape[1])])
#        table_alpha.field_names = tmp.copy()
#        for i, tmpLi in enumerate(np.round(alphaAll, 5)):
#            tmp = [ str(i+1) ]
##            print(tmpLi[0, :].copy())
#            tmp.extend(np.squeeze(np.array(tmpLi)))
#            table_alpha.add_row(tmp)
##        table_alpha.add_row(["="*6 for i in range(Y.shape[1]+1)])
##        tmp = ['residual']
##        tmp.extend(residual)
##        table_alpha.add_row(tmp)
#        print(table_alpha)
...
#            table_residual.add_row([str(i+1), Y[:, i].T, Y_hat[:, i].T, residual[i]])
#        
#        print(table_residual)
        def ShowAnswer(alpha, residual, data, data_hat, boolRound = True, intRoundDigits = 5):
            """ """
        #    roundFunc = (lambda x : np.round(x, intRoundDigits)) if boolRound else (lambda x: x)
            if boolRound:
