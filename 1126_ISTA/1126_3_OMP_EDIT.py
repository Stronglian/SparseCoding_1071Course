...
"""
"""
所以網路演算法 跟 老師 中段寫的 有什麼不同?
下次來 Orthonormal bases 
"""
#%%
import numpy as np
...
class OMP(): #MP
    def __init__(self, dictInput, signalInput, letter):
        self.D = dictInput.copy()
        self.X = signalInput.copy()
        self.L = letter
...
if __name__ == "__main__":
    ...
    X_hat, residual = omp.VarifyAll(alpha_X_All, boolRound = boolRound)
    #%% 表格顯示
    ...
    table = PrettyTable()
    tmp = ['#']
    tmp.extend(["x"+str(i+1)+"'s SC" for i in range(alpha_X_All.shape[1])])
    ...
    # 顯示 - residual
    table2 = PrettyTable()
    table2.field_names = ["#", "原本", "X_hat", "residual"]
    for i in range(X.shape[1]):
        table2.add_row(["x"+str(i+1)+"'s SC", X[:, i], X_hat[:, i], residual[i]])
